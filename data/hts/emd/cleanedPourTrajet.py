...
def execute(context):
    ...
    ## Accessement of intermodal trips
    for prefix, mode in MODES_MECA_MAP:
        df_trips.loc[
            df_trips["modeM"].astype(np.str).str.startswith(prefix), "modeM"
        ] = mode
    ...
    # Fix activity types (because of inconsistent EMD data and removing in the timing fixing step)
